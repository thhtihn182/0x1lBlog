package top.blogapi.util.markdown.ext.cover;

import org.commonmark.Extension;
import org.commonmark.parser.Parser;
import org.commonmark.renderer.html.HtmlNodeRendererFactory;
import org.commonmark.renderer.html.HtmlRenderer;
import org.commonmark.renderer.text.TextContentNodeRendererFactory;
import org.commonmark.renderer.text.TextContentRenderer;
import top.blogapi.util.markdown.ext.cover.internal.CoverDelimiterProcessor;
import top.blogapi.util.markdown.ext.cover.internal.CoverHtmlNodeRenderer;
import top.blogapi.util.markdown.ext.cover.internal.CoverTextContentNodeRenderer;

public class CoverExtension implements Parser.ParserExtension, HtmlRenderer.HtmlRendererExtension, TextContentRenderer.TextContentRendererExtension {

    private CoverExtension(){}
    public static Extension create(){
        return new CoverExtension();
    }

    @Override
    public void extend(Parser.Builder builder) {
        builder.customDelimiterProcessor(new CoverDelimiterProcessor());
    }

    @Override
    public void extend(HtmlRenderer.Builder builder) {
        builder.nodeRendererFactory((HtmlNodeRendererFactory) CoverHtmlNodeRenderer::new);
    }

    @Override
    public void extend(TextContentRenderer.Builder builder) {
        builder.nodeRendererFactory((TextContentNodeRendererFactory) CoverTextContentNodeRenderer::new);
    }
}
